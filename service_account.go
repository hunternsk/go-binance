...
func (as *apiService) WithdrawHistory(hr HistoryRequest) ([]*Withdrawal, error) {
	...
	for _, w := range rawWithdrawHistory.WithdrawList {
		...
		w.AmountFloat, err = strconv.ParseFloat(w.Amount, 64)
		if err != nil {
			return nil, err
		}
		w.TransactionFeeFloat, err = strconv.ParseFloat(w.TransactionFee, 64)
		if err != nil {
			return nil, err
		}
		wc = append(wc, &Withdrawal{
			Id:             w.Id,
			Amount:         w.AmountFloat,
			TransactionFee: w.TransactionFeeFloat,
			Address:        w.Address,
			TxID:           w.TxID,
			Asset:          w.Asset,
			ApplyTime:      t,
